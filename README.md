...
커밋은 다음과 같이 분류하여 작성한다.

- **feat** : 기능 개발
- **fix** : 버그 수정
- **style** : CSS 스타일 수정
- **docs** : 문서 작업 (주로 README 작성 등)
- **refactor** : 기능 변경 없이 구조/로직 개선
- **chore** : 파일 이동, 이름 변경, 주석 추가 등 단순 작업
...
1. 커밋 메시지는 **반드시 한글로 작성**한다.
2. 커밋 유형은 **영문 소문자**로 작성한다.
3. 필요 시 메시지 앞에 `[#이슈번호]`를 포함한다.
4. 커밋은 **논리적으로 독립적인 작업 단위**로 작성한다.
   - 독립적으로 빌드 및 테스트 가능해야 한다.
   - 롤백 시 최소한의 영향을 주는 단위여야 한다.
...
- 오늘의 질문 → 답변 작성 → 매칭 → 10회 교환 → 종료 리뷰
- D+3 편지 수신 알림 → 보관함 확인 → 주간 리포트

<!-- ![flow gif](./assets/demo.gif) -->
